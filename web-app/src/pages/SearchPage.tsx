...
const SearchPage: React.FC = () => {
  ...
  return (
    <div className="page">
      ...
      {result === null && (
        <div className="search-empty card" data-testid="search-empty">
          <p>No vehicle found for plate <strong>{plate}</strong>.</p>
        </div>
      )}

      {result && targetVehicle && (
        <div className="card search-result" data-testid="search-result">
          <VehicleSilhouette color={targetVehicle.color} size={100} />
          <div className="search-result-info">
            <p className="search-plate" data-testid="search-plate-text">{targetVehicle.licensePlate}</p>
            <p className="search-vehicle" data-testid="search-vehicle-text">{targetVehicle.make} {targetVehicle.model}</p>
            <p className="search-color">{targetVehicle.color}</p>
          </div>
          <div className="search-actions">
            <button
              className="btn-primary"
              data-testid="search-send-btn"
              onClick={() => navigate(
                `/compose?recipientId=${result.uid}&vehicleId=${targetVehicle.id}&plate=${targetVehicle.licensePlate}&isFriend=${isFriend}`
              )}
            >
              Send Message
            </button>
            {!isFriend && (
              <button
                ...
                data-testid="search-friend-btn"
                onClick={() => navigate(
                  `/compose?recipientId=${result.uid}&vehicleId=${targetVehicle.id}&plate=${targetVehicle.licensePlate}&friendRequest=true`
                )}
              >
                Add as Friend
              </button>
